...
| T07 | En cours | Pilotage ARDY, postures, arrêt et reprise réels ; correction des jambes et rejet des semelles glissantes. La série 07 termine deux postures et refuse deux déplacements avant lecture. [Mesures et limites](motion-validation.md) |
| T08 | À réaliser | Interactions corporelles avec les objets non livrées |
| T09 | Pont MCP et construction restreinte vérifiés, conversation à réaliser | Vrai agent Hermes construit avec exactement cinq outils et sans contexte projet ni modèle de secours ; commande réelle ARDY et retransmission vérifiées. Tours persistés en v5, mutations tardives refusées dans le transport MCP testé. Accès OpenAI essayé : HTTP 401. [Configuration et limites](hermes-setup.md) |
