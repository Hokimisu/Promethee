...
Le mode résident, proposé lors de l'audit, est maintenant livré sur activation explicite et qualifié sur trois tours natifs Luna, puis sur [une session complète dans la scène voix/corps](11-short-dialogue.md#scène-livrée-avec-hermes-résident). Il garde l'agent, ferme puis reconnecte MCP pour chaque tour. Le préchauffage jetable reste le choix par défaut de l'essai vocal. Les mesures natives détaillées figurent dans le [rapport de cycle de vie](12-hermes-lifecycle.md#mode-résident-livré-et-qualification-native).
...
2. **Exécution simultanée locale des outils MCP** nécessite `supports_parallel_tool_calls: true` au niveau du serveur. Notre profil ne le définit pas ; le défaut natif est faux. Ce réglage concerne tout le serveur, pas seulement les annotations read-only. Ne pas l'activer globalement sur un serveur mêlant lecture, soumission et annulation. Les dépendances `read_world → submit_action` restent séquentielles ; leur garde pendant la présence est désormais traitée par le contrat de révision de commande, pas par la parallélisation.
...
Lors de l'audit initial, les observations de présence périmaient la révision entre lecture et soumission. Le [contrat de révision de commande](../command-revision.md) a depuis corrigé cette frontière : `read_world` expose `command_revision` et `submit_action` accepte `expected_command_revision`, ou l'ancienne garde stricte `expected_revision`, avec exactement une garde par demande. Les cinq outils restent les mêmes ; la validation conserve le tour de conversation et vérifie l'action sur le monde courant. Aucun rendez-vous qui gèle la présence pendant la réflexion, ni remplacement silencieux de révision, n'est ajouté.

L'essai réel `command-revision-real-01` utilise Hermes 0.21.3 G `2179a279`, Luna résident et une présence ARDY réelle. La dernière lecture `revision=203, command_revision=3` conduit à une admission `247/3`, après 44 poses distinctes archivées ; la posture termine 7,928 s après admission. La première demande plaçait `name` directement dans `action` au lieu de `action.args` : elle est refusée, puis corrigée par Hermes dans le même tour avec un nouvel ID. Une seule action est acceptée, sans rejeu automatique de l'hôte. Le tour suivant utilise réellement `read_world` et `read_execution` pour relire le résultat.

Ces deux tours prennent 18,82 et 8,02 s. Ils qualifient le passage présence → commande pour cette posture, sans voix ni navigateur ; ils ne prouvent ni fluidité générale, ni synchronisation sémantique gestes/paroles. Microphone, coût réel, budget vocal persistant et marche avec objet tenu restent hors de cet essai. Les preuves structurées sont `audit.json` et `report.json` sous `.local/command-revision-real-01/` ; aucun raisonnement natif n'est reproduit ici.
...
Une batterie CPU a démarré pendant la qualification : ce n'est pas une comparaison isolée avec le préchauffage jetable. Les reconnexions MCP des tours réutilisés prennent **1,642 et 1,863 s** et restent dans le chemin critique. La réutilisation effective est prouvée ; aucun gain de latence, de facturation ou de cache fournisseur n'est déduit de ces trois observations. Les JSON `{text, delivery}` sont validés, mais cet essai ne synthétise aucune voix et ne qualifie pas à lui seul la scène complète. Le refus des drapeaux natifs `failed`, `partial` et `completed=False` a ensuite été renforcé sur CPU, sans nouvel appel modèle à cette étape.

Une [session de scène ultérieure](11-short-dialogue.md#scène-livrée-avec-hermes-résident), `dialogue-trial-5d84d867`, a depuis terminé avec lecture et poses observées, sans sous-alimentation ni erreur. Elle suit un premier essai dont le navigateur avait cessé la lecture alors que le serveur continuait. La correction HTTP/1.1 intervient également entre ces essais : leur résultat ne démontre pas un gain A/B pur ni un débit garanti. Le rapport lié conserve le détail des lectures terminées et de la dernière réponse coupée à 60 secondes.
...
| 2 | Conserver le préchauffage jetable par défaut de l'essai vocal ; utiliser le résident sur activation explicite dans son périmètre qualifié. | Les deux chemins sont livrés. Le jetable active son identifiant réservé avec historique frais. Le résident est activé par `--resident` / `args.resident=True`, avec authentification native Hermes Codex ; il reconnecte MCP et revalide les schémas à chaque tour. Trois tours natifs et une session complète de scène sont qualifiés, sans garantie de débit. Aucun appel modèle avant activation. `host.cancel()` invalide et détruit un tour actif, mais conserve une réserve inactive ou un résident au repos ; `host.close()` les détruit aussi. Le résident expire après 120 s d'inactivité par défaut et `warm()` ne crée pas de second agent pendant son tour. Instrumentation du cache fournisseur encore à faire. |
...
| 5 | Rendre les réponses des **outils existants** plus compactes pour le dialogue, sans modifier leurs reçus persistés. | Intégré ensuite : lectures `summary` par défaut et `full` explicite, seules les matrices de pose et l'apparence sont omises. Révisions, objets, attachements, statuts, erreurs et provenance conservés. Le [rapport de qualification](14-hermes-tool-latency.md) distingue réduction d'octets, essais natifs et limites de latence. Aucun nombre de tokens n'est déduit des caractères. |
